...
do { \
	...
	cu_assert_fails = 0; \
	cu_case_runs++; \
	printf("  %-9s  Case: %d  File: %s  Function: %s\n", \
                "Test Case", \
				cu_case_runs, \
                __FILE__, \
                #cu_case_func); \
...
do { \
	cu_assert_runs++; \
	if (!(cu_assert_expr)) { \
		printf("    %-9s Assertion %d.%d (%s)  Message: %s  File: %s  Line: %d\n", \
                "False", \
                cu_case_runs, \
                cu_assert_runs, \
                #cu_assert_expr, \
		...
		printf("    %-9s Assertion %d.%d (%s)  File: %s  Line: %d\n", \
                "True", \
                cu_case_runs, \
                cu_assert_runs, \
                #cu_assert_expr, \
...
do { \
	printf("%-9s Test suite.  File: %s.  Function: %s.  ", \
				...
				__FUNCTION__); \
	if (cu_case_fails == 0)  { \
		printf("%d of %d functions (%3.1f%%) passed.", \
				cu_case_runs, \
				cu_case_runs, \
				100.0); \
	} \
	else { \
		printf("%d of %d functions (%3.1f%%) failed.", \
				cu_case_fails, \
				cu_case_runs, \
...
/**
 Determine the return value of the test suite. 
   1 means that at least one failure occurred in the suite.
