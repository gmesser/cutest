...
do { \
	cu_assert_runs++; \
	if (!(cu_assert_expr)) { \
		printf("    %-9s Assertion #%d.%d (%s) %s\n", "False", cu_case_runs, cu_assert_runs, #cu_assert_expr, #cu_failure_message); \
		cu_assert_fails++; \
	} \
	else { \
		printf("    %-9s Assertion #%d.%d (%s)\n", "True", cu_case_runs, cu_assert_runs, #cu_assert_expr); \
	} \
} while (0)

/*
 Run a test case function and report the result.  
   Passed means all the assertions in the test function were true.
   Failed means one or more of the assertions in the test function were false.
*/
...
do { \
	...
	if (cu_assert_fails == 0) { \
		printf("%d of %d (%3.1f%%) assertions were true.", \
					cu_assert_runs, \
					cu_assert_runs, \
					100.0); \
	} \
 	else { \
		printf("%d of %d (%3.1f%%) assertions were false.", \
					cu_assert_fails, \
					cu_assert_runs, \
...
/*
 Determine the current return value of the test suite. 
